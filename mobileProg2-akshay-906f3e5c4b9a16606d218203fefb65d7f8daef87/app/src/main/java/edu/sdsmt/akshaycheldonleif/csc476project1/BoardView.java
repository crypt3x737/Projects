...
public class BoardView extends View {
    ...
    public void setRegions(ArrayList<Integer> regions) {
        for (int i = 0; i < regions.size(); i++) {
            this.regions.get(i).ownership = regions.get(i);
            if (regions.get(i) == constants.noPlayer) {
                this.regions.get(i).myColor = Color.YELLOW;
            } else if (regions.get(i) == constants.player1) {
                this.regions.get(i).myColor = Color.RED;
            } else if (regions.get(i) == constants.player2) {
                this.regions.get(i).myColor = Color.BLUE;
            }
        }
    }
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        game.addValueEventListener(new ValueEventListener(){
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                for(int i = 0; i < 25; i++) {
                    //regions.get(i).ownership = Integer.parseInt(dataSnapshot.child("BoardState").child(Integer.toString(i)).getValue().toString());
                }
            }
            ...
            if(capOp == 0)
            {
                if(captureX < temp.right && captureX > temp.left)
                {
                    if(captureY < temp.bot && captureY > temp.top)
                    {
                        ...
                        if(playerTurn == 1)
                        {
                            game.child("BoardState").child(Integer.toString(i)).setValue(1);
                        ...
                        else
                        {
                            ...
                            game.child("BoardState").child(Integer.toString(i)).setValue(0);
